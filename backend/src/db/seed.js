...
async function seed() {
  ...
  const [adminResult] = await db.query(
    ...
    [
      "Admin User",
      "admin@alphaclasses.com",
      hash,
      "Alpha Classes Private Limited",
  ...
  /* ── 2. Teachers ─────────────────────────────────────── */
  const teachers = [
    { name: "Dr. Anil Mehta",   email: "anil@alphaclasses.com",   phone: "9876543220", location: "Main Branch", subjects: ["Mathematics","Physics"] },
    { name: "Mrs. Sunita Rao",  email: "sunita@alphaclasses.com",  phone: "9876543221", location: "Main Branch",     subjects: ["Chemistry","Biology"] },
    { name: "Mr. Rajesh Kumar", email: "rajesh@alphaclasses.com",  phone: "9876543222", location: "Main Branch",  subjects: ["English","History"] },
  ];
  const teacherHash = await bcrypt.hash("teacher123", 10);
  const teacherIds = [];
  for (const t of teachers) {
    const [r] = await db.query(
      `INSERT INTO teachers (admin_id, name, email, phone, password, institute, location, subjects)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [adminId, t.name, t.email, t.phone, teacherHash, "Alpha Classes Pvt Ltd", t.location, JSON.stringify(t.subjects)]
    );
  ...
  /* ── 3. Branches ─────────────────────────────────────── */
  const [brResult1] = await db.query("INSERT INTO branches (branch_name) VALUES ('Main Branch')");
  const [brResult2] = await db.query("INSERT INTO branches (branch_name) VALUES ('Wakad')");
  const [brResult3] = await db.query("INSERT INTO branches (branch_name) VALUES ('Nigdi')");
  ...
  await db.query(
    `INSERT INTO inquiry_extra (name, phone, father_name, father_phone, dob, sex, email, address, standard, course, board, location, last_exam_marks, college_name, college_timing, future_plans, father_occupation, mother_occupation, sibling_name, reference, taking_coaching, hostel_required, inquiry_date)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      "Arjun Singh", "9876540001", "Vikram Singh", "9876540002", "2010-05-15", "Male", "arjun@example.com", "Chinchwad, Pune", "10", "Science Tuition", "CBSE", "Main Branch", "85%", "Vidya Niketan", "Morning", "Engineering", "Business", "Homemaker", "None", "Friend", "No", "No", new Date()
    ]
  );
  ...
  /* ── 11. Students ────────────────────────────────────── */
  const students = [
    { name:"Rahul Sharma",  email:"rahul@example.com",  phone:"9876543210", father_name:"Suresh Sharma",  father_phone:"9876543211", board:"CBSE",  standard:"10", course:"Science",  location:"Main Branch", fee:5000, paid_fee:5000 },
    { name:"Priya Patel",   email:"priya@example.com",  phone:"9876543212", father_name:"Rajesh Patel",   father_phone:"9876543213", board:"ICSE",  standard:"9",  course:"Commerce", location:"Main Branch",     fee:5000, paid_fee:2500 },
    { name:"Amit Kumar",    email:"amit@example.com",   phone:"9876543214", father_name:"Vikram Kumar",   father_phone:"9876543215", board:"State", standard:"11", course:"Science",  location:"Main Branch",  fee:6000, paid_fee:6000 },
    { name:"Sneha Joshi",   email:"sneha@example.com",  phone:"9876543216", father_name:"Prakash Joshi",  father_phone:"9876543217", board:"CBSE",  standard:"12", course:"Arts",     location:"Main Branch", fee:7000, paid_fee:0    },
  ];
  ...
  for (const s of students) {
    const [r] = await db.query(
      `INSERT INTO students
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,0)`,
      [adminId,s.name,s.email,studentHash,s.phone,s.father_name,s.father_phone,s.board,s.standard,s.course,s.location,"Alpha Classes Pvt Ltd",s.fee,s.paid_fee]
    );
  ...
  await db.query(
    `INSERT INTO inquiries (admin_id,name,phone,father_name,father_phone,course,location,board,standard,status,video,inquiry_date)
     ...
     (?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      adminId,"Arjun Singh",  "9876540001","Vikram Singh","9876540002","Science Tuition","Main Branch","CBSE", "10","New",      "", d(0),
      adminId,"Neha Desai",   "9876540003","Mohan Desai", "9876540004","Math Classes",   "Main Branch",    "ICSE", "9", "Contacted","", d(-1),
    ]
  );
  ...
  await db.query(
    `INSERT INTO appointments (admin_id,name,standard,board,course,appointment_date,appointment_time,location,whatsapp,status)
     ...
     (?,?,?,?,?,?,?,?,?,?)`,
    [
      adminId,"Rahul Sharma","10","CBSE","Science Tuition", d(0), "10:00","Main Branch","+919876543210","Pending",
      adminId,"Priya Patel", "9", "ICSE","Math Classes",    d(1), "14:00","Main Branch",    "+919876543212","Confirmed",
    ]
  );
  ...
  console.log("   Email    : admin@alphaclasses.com");
  ...
  console.log("   Email    : anil@alphaclasses.com (or any teacher email)");
